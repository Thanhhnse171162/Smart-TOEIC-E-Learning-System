...
import {
  TrendingUp, Target, Users, BarChart3, ArrowUpRight, ArrowDownRight,
  ...
  return (
    <DashboardLayout sidebarItems={teacherSidebarItems} title="Analytics" subtitle="Student performance insights & question analysis" userName="Tran Thi B">
      <div className="max-w-[1400px] mx-auto pb-10 space-y-6">
        ...
        {/* Charts Row 2: Part Accuracy & Difficult Questions */}
        <div className="grid gap-6 lg:grid-cols-2">
          {/* Part Performance (Rebuilt using HTML for clean look) */}
          <Card className="rounded-2xl border-slate-100 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-[15px] font-extrabold text-slate-800">Accuracy by Part</CardTitle>
              <p className="text-xs font-medium text-slate-500">Identify which TOEIC parts your students struggle with</p>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {partPerformance.map((item) => {
                  const color = item.accuracy >= 75 ? 'bg-emerald-500' : item.accuracy >= 60 ? 'bg-amber-500' : 'bg-rose-500';
                  const bgLight = item.accuracy >= 75 ? 'bg-emerald-50' : item.accuracy >= 60 ? 'bg-amber-50' : 'bg-rose-50';
                  const textColor = item.accuracy >= 75 ? 'text-emerald-700' : item.accuracy >= 60 ? 'text-amber-700' : 'text-rose-700';
                  
                  return (
                    <div key={item.part} className="space-y-1.5">
                      <div className="flex items-center justify-between text-[13px] font-bold">
                        <span className="text-slate-700">{item.part}: <span className="text-slate-500 font-semibold">{item.label}</span></span>
                        <span className={`${textColor}`}>{item.accuracy}%</span>
                      </div>
                      <div className={`w-full h-3 rounded-full ${bgLight} overflow-hidden border border-slate-100`}>
                         <div className={`h-full rounded-full ${color} transition-all duration-500`} style={{ width: `${item.accuracy}%` }} />
                      </div>
                    </div>
                  );
                })}
              </div>
              <div className="flex items-center justify-center gap-5 mt-6 pt-4 border-t border-slate-100 text-[11px] font-bold">
                ...
                <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-rose-500"></span> &lt;60% (Review)</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-2xl border-slate-100 shadow-sm">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-[15px] font-extrabold text-slate-800">Review Targets</CardTitle>
                  <p className="text-xs font-medium text-slate-500">Questions with the highest failure rate</p>
                </div>
                <Badge className="bg-rose-50 text-rose-600 border border-rose-100 shadow-none text-[10px] font-bold px-2 py-0.5">
            ...
            <CardContent className="space-y-3">
              {difficultQuestions.map((q, i) => (
                <div key={q.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:border-rose-100 hover:bg-rose-50/30 transition-all cursor-pointer group">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[12px] font-black shrink-0 ${
                    i === 0 ? 'bg-rose-500 text-white shadow-sm' : i === 1 ? 'bg-rose-400 text-white shadow-sm' : 'bg-rose-100 text-rose-600'
                  }`}>
                    {i + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12.5px] font-bold text-slate-700 mt-1 truncate group-hover:text-rose-700 transition-colors">{q.text}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-[15px] font-black text-rose-600">{q.errorRate}% fail</p>
                    <p className="text-[10px] text-slate-400 font-medium">{q.attempts} attempts</p>
                  </div>
                </div>
              ))}
              <Button variant="ghost" className="w-full mt-2 text-indigo-600 font-bold hover:text-indigo-700 hover:bg-indigo-50 border border-dashed border-slate-200">
                 View All Weaknesses
              </Button>
            </CardContent>
          </Card>
